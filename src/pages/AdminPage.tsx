...
const ANGEBOTE_FIELDS = [
  ...
  { key: 'stammdaten_ref', label: 'Stammdaten-Referenz', type: 'applookup/select', targetEntity: 'stammdaten', targetAppId: 'STAMMDATEN', displayField: 'kunden_csv' },
...
export default function AdminPage() {
  ...
  const getApplookupDisplay = useCallback((entity: EntityKey, fieldKey: string, url?: unknown) => {
    ...
    if (entity === 'angebote' && fieldKey === 'stammdaten_ref') {
      const match = (lists.stammdatenList ?? []).find((r: any) => r.record_id === id);
      return match?.fields.kunden_csv ?? '—';
    }
    return String(url);
  }, [getLookupLists]);
